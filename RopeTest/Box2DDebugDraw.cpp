...
static CCPoint Convert(const b2Vec2& orgPoint)
{
   CCPoint point(orgPoint.x*PTM_RATIO,orgPoint.y*PTM_RATIO);
   return point;
}
...
void Box2dDebugDraw::DrawPolygon(const b2Vec2* old_vertices, int32 vertexCount, const b2Color& color)
{
   ...
   {
      CCPoint tmp = Convert(old_vertices[i]);
      vertices[i].x = tmp.x;
      vertices[i].y = tmp.y;
   }
...
void Box2dDebugDraw::DrawAABB(b2AABB* aabb, const b2Color& color)
{
   ...
   CCPoint lowerBound = Convert(aabb->lowerBound);
   CCPoint upperBound = Convert(aabb->upperBound);
...
void Box2dDebugDraw::DrawCircle(const b2Vec2& center, float32 radius, const b2Color& color)
{
   ...
   {
      CCPoint v = Convert(center + radius * b2Vec2(cosf(theta), sinf(theta)));
      glVertices[i*2]=v.x;
...
void Box2dDebugDraw::DrawSegment(const b2Vec2& p1Org, const b2Vec2& p2Org, const b2Color& color)
{
   ...
   CCPoint p1 = Convert(p1Org);
   CCPoint p2 = Convert(p2Org);
